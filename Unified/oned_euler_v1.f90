...
!      if (i <= ncells/2) then
       ...
       cell(i)%w(1) = 0.964024984265400+(1/dx)*(0.01/(2*acos(-1.0_p2)))*(cos(2*acos(-1.0_p2)*(xmin+(real(i))*dx)) &
-cos(2*acos(-1.0_p2)*(xmin+(real(i-1))*dx)))!1-0.1*(xmin+real(i-1)*dx)  !Density  on the left
       cell(i)%w(2) = 0.319121501427128+(1/dx)*(0.01/(2*acos(-1.0_p2)))*(cos(2*acos(-1.0_p2)*(xmin+(real(i))*dx)) &
-cos(2*acos(-1.0_p2)*(xmin+(real(i-1))*dx)))!0.0_p2   !Velocity on the left
       cell(i)%w(3) = 0.95+ (1/dx)*(0.01/(2*acos(-1.0_p2)))*(cos(2*acos(-1.0_p2)*(xmin+(real(i))*dx)) &
-cos(2*acos(-1.0_p2)*(xmin+(real(i-1))*dx)))!1+0.1*(xmin+real(i-1)*dx)   !Pressure on the left
 !     else
  !     cell(i)%w(1) = 0.964024984265400+0.01*(-sin(2*acos(-1.0_p2)*(xmin+real(i-1)*dx)))!1-0.1*(xmin+real(i-1)*dx)!Density  on the right
   !    cell(i)%w(2) = 0.319121501427128+0.01*(-sin(2*acos(-1.0_p2)*(xmin+real(i-1)*dx)))!0.0_p2   !Velocity on the right
    !   cell(i)%w(3) = 0.95+0.01*(-sin(2*acos(-1.0_p2)*(xmin+real(i-1)*dx)))!1+0.1*(xmin+real(i-1)*dx) !1.0_p2   !Pressure on the right
     ! endif
...
!cell(2)%u(3) = 5.9;
!do i=1,2
! cell(i)%w = u2w(cell(i)%u,gamma)
!end do
!wL = cell(1)%w;
!wR = cell(2)%w;
!write(*,*) roe_flux(wL,wR,gamma)

cell(1)%wL(1) = 0.961539830123639
cell(2)%wL(1) = 0.955801978872319
cell(3)%wL(1) = 0.954188667834565
cell(4)%wL(1) = 0.954627667126845
cell(5)%wL(1) = 0.958656122162991
cell(6)%wL(1) = 0.964735300041346
cell(7)%wL(1) = 0.970543161436035
cell(8)%wL(1) = 0.973861300696236
cell(9)%wL(1) = 0.971717473181509
cell(10)%wL(1) = 0.967619008001995

cell(1)%wL(2) = 0.316636347285367
cell(2)%wL(2) = 0.310898496034047
cell(3)%wL(2) = 0.309285184996293
cell(4)%wL(2) =    0.309724184288573
cell(5)%wL(2) = 0.313752639324719
cell(6)%wL(2) =    0.319831817203074
cell(7)%wL(2) = 0.325639678597763
cell(8)%wL(2) =    0.328957817857963
cell(9)%wL(2) = 0.326813990343237
cell(10)%wL(2) =    0.322715525163722

cell(1)%wL(3) = 0.947514845858239
cell(2)%wL(3) = 0.941776994606919
cell(3)%wL(3) = 0.940163683569165
cell(4)%wL(3) = 0.940602682861445
cell(5)%wL(3) = 0.944631137897591
cell(6)%wL(3) = 0.950710315775946
cell(7)%wL(3) = 0.956518177170636
cell(8)%wL(3) = 0.959836316430836
cell(9)%wL(3) = 0.957692488916109
cell(10)%wL(3) = 0.953594023736595




cell(1)%wR(1) = 0.960430960528806
cell(2)%wR(1) = 0.956332495349292
cell(3)%wR(1) = 0.954188667834565
cell(4)%wR(1) = 0.957506807094765
cell(5)%wR(1) = 0.963314668489454
cell(6)%wR(1) = 0.969393846367809
cell(7)%wR(1) = 0.973422301403956
cell(8)%wR(1) = 0.973861300696236
cell(9)%wR(1) = 0.972247989658482
cell(10)%wR(1) = 0.966510138407162



cell(1)%wR(2) =  0.315527477690534
cell(2)%wR(2) =  0.311429012511020
cell(3)%wR(2) =  0.309285184996293
cell(4)%wR(2) =  0.312603324256493
cell(5)%wR(2) =  0.318411185651182
cell(6)%wR(2) =  0.324490363529537
cell(7)%wR(2) =   0.328518818565683
cell(8)%wR(2) =   0.328957817857963
cell(9)%wR(2) =   0.327344506820210
cell(10)%wR(2) =   0.321606655568889


cell(1)%wR(3) = 0.946405976263406
cell(2)%wR(3) =    0.942307511083892
cell(3)%wR(3) = 0.940163683569165
cell(4)%wR(3) =    0.943481822829365
cell(5)%wR(3) = 0.949289684224054
cell(6)%wR(3) =    0.955368862102409
cell(7)%wR(3) = 0.959397317138556
cell(8)%wR(3) =    0.959836316430836
cell(9)%wR(3) = 0.958223005393082
cell(10)%wR(3) =    0.952485154141762
