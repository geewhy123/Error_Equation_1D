...
% if(l == 1)
% [errerr(i,j,k,l),x,cverr(i,j,k,l),exacterr(i,j,k,:),ee(i,j,k,:)] = errordriver(N(l),pp,qq,rr,1/3,'P',0,'P',0,0.1,tt,'Poisson','TimeAccurate');
% % [errerr(i,j),x,cverr(i,j),exacterr(i,j,:),ee(i,j,:)] = errordriver(N(l),pp,qq,rr,1/3,'P',0,'P',0,0.1,tt,'Poisson','TimeAccurate');
% else
%     errerr(i,j,k,l) = errordriver(N(l),pp,qq,rr,1/3,'P',0,'P',0,0.1,tt,'Poisson','TimeAccurate');
% end
%             end
%         end
%     end
...
% save('unsteadytestfullprofilesdiffusion3.mat','errerr','x','cverr','exacterr','ee')
% error('1')
