...
M = [1 1 1 1 ; 
     ...
     %      M = [1 1 1 1 1; 
%          ((h(i+1)+h(i+2)+h(i+3))^2-(h(i+1)+h(i+2))^2)/(2*h(i+3))  ((h(i+1)+h(i+2))^2-h(i+1)^2)/(2*h(i+2)) h(i+1)/2 -h(i)/2 ((-h(i))^2-(-h(i)-h(i-1))^2)/(2*h(i-1));
%          ((h(i+1)+h(i+2)+h(i+3))^3-(h(i+1)+h(i+2))^3)/(6*h(i+3))  ((h(i+1)+h(i+2))^3-h(i+1)^3)/(6*h(i+2)) h(i+1)^2/6  h(i)^2/6  ((-h(i))^3-(-h(i)-h(i-1))^3)/(6*h(i-1));
%          ((h(i+1)+h(i+2)+h(i+3))^4-(h(i+1)+h(i+2))^4)/(24*h(i+3))  ((h(i+1)+h(i+2))^4-h(i+1)^4)/(24*h(i+2)) h(i+1)^3/24 -h(i)^3/24 ((-h(i))^4-(-h(i)-h(i-1))^4)/(24*h(i-1));
%          ((h(i+1)+h(i+2)+h(i+3))^5-(h(i+1)+h(i+2))^5)/(120*h(i+3)) ((h(i+1)+h(i+2))^5-h(i+1)^5)/(120*h(i+2)) h(i+1)^4/120 h(i)^4/120 ((-h(i))^5-(-h(i)-h(i-1))^5)/(120*h(i-1))];
      b = [ 0 1 0 0 ]';
...
v = M\b;
L = [1 1 1 1 ; 
      ((h(i+1)+h(i))^2-h(i)^2)/(2*h(i+1)) h(i)/2 -h(i-1)/2    ((-h(i-1))^2-(-h(i-1)-h(i-2))^2)/(2*h(i-2)) ;
      ((h(i+1)+h(i))^3-h(i)^3)/(6*h(i+1)) h(i)^2/6 h(i-1)^2/6 ((-h(i-1))^3-(-h(i-1)-h(i-2))^3)/(6*h(i-2));
      ((h(i+1)+h(i))^4-h(i)^4)/(24*h(i+1)) h(i)^3/24 -h(i-1)^3/24     ((-h(i-1))^4-(-h(i-1)-h(i-2))^4)/(24*h(i-2))];
...
q = q/h(i);
 ...
 %         if i==z
%              uea
%         a= v(1)*uea(i+2)+v(2)*uea(i+1)+v(3)*uea(i)+v(4)*uea(i-1)
%         b = w(1)*uea(i+1)+w(2)*uea(i)+w(3)*uea(i-1)+w(4)*uea(i-2)
%         c = (a-b)/h(z)
%          X = x(z)+h(z)/2;
%          Y = x(z-1)+h(z-1)/2;
%          A = -2*X*exp(-X^2)
%          B = -2*Y*exp(-Y^2)
%          syms p
%          C=double((1/(X-Y))*int((-2+4*p^2)*exp(-p^2),p,Y,X))
%          abs(a-A)
%          abs(b-B)
%          abs(c-C)
  %    end
    ...
    s(i) = (q(1)*u(i+2)+q(2)*u(i+1)+q(3)*u(i)+q(4)*u(i-1)+q(5)*u(i-2));%+q(6)*u(i-2));
   
    
    d(i) = (q(1)*uea(i+2)+q(2)*uea(i+1)+q(3)*uea(i)+q(4)*uea(i-1)+q(5)*uea(i-2));
    end
    %if ((i > 1) && (i < N+2))
    %s(i) =  (u(i-1)-2*u(i)+u(i+1))/(h^2);
    %end
...
L = [ 1 1 1 1 1;
    ((h(j)+h(j+1)+h(j+2)+h(j+3))^2-(h(j)+h(j+1)+h(j+2))^2)/(2*h(j+3))   ((h(j)+h(j+1)+h(j+2))^2-(h(j)+h(j+1))^2)/(2*h(j+2))   ((h(j)+h(j+1))^2-(h(j))^2)/(2*h(j+1))  h(j)/2 -h(j-1)/2;
    ((h(j)+h(j+1)+h(j+2)+h(j+3))^3-(h(j)+h(j+1)+h(j+2))^3)/(6*h(j+3))   ((h(j)+h(j+1)+h(j+2))^3-(h(j)+h(j+1))^3)/(6*h(j+2))   ((h(j)+h(j+1))^3-(h(j))^3)/(6*h(j+1))  h(j)^2/6 h(j-1)^2/6;
    ((h(j)+h(j+1)+h(j+2)+h(j+3))^4-(h(j)+h(j+1)+h(j+2))^4)/(24*h(j+3))  ((h(j)+h(j+1)+h(j+2))^4-(h(j)+h(j+1))^4)/(24*h(j+2))  ((h(j)+h(j+1))^4-(h(j))^4)/(24*h(j+1)) h(j)^3/24 -h(j-1)^3/24;
    ((h(j)+h(j+1)+h(j+2)+h(j+3))^5-(h(j)+h(j+1)+h(j+2))^5)/(120*h(j+3))  ((h(j)+h(j+1)+h(j+2))^5-(h(j)+h(j+1))^5)/(120*h(j+2))  ((h(j)+h(j+1))^5-(h(j))^5)/(120*h(j+1)) h(j)^4/120 h(j-1)^4/120];
w = L\b;
...
d(2) = (q(1)*uea(1)+q(2)*uea(2)+q(3)*uea(3)+q(4)*uea(4)+q(5)*uea(5)+q(6)*uea(6));
...
j=2;
        ...
        a = v(1)*uea(j+4)+v(2)*uea(j+3)+v(3)*uea(j+2)+v(4)*uea(j+1)+v(5)*uea(j)
        b = w(1)*uea(j+3)+w(2)*uea(j+2)+w(3)*uea(j+1)+w(4)*uea(j)+w(5)*uea(j-1)
        c = (a-b)/h(j)
         X = x(j)+h(j)/2;
         Y = x(j-1)+h(j-1)/2;
         AA = -2*X*exp(-X^2)
...
L = [ 1 1 1 1 1;
    ((-h(j)-h(j-1)-h(j-2))^2-(-h(j)-h(j-1)-h(j-2)-h(j-3))^2)/(2*h(j-3))    ((-h(j)-h(j-1))^2-(-h(j)-h(j-1)-h(j-2))^2)/(2*h(j-2))   ((-h(j))^2-(-h(j)-h(j-1))^2)/(2*h(j-1))  -h(j)/2 h(j+1)/2;
    ((-h(j)-h(j-1)-h(j-2))^3-(-h(j)-h(j-1)-h(j-2)-h(j-3))^3)/(6*h(j-3))    ((-h(j)-h(j-1))^3-(-h(j)-h(j-1)-h(j-2))^3)/(6*h(j-2))   ((-h(j))^3-(-h(j)-h(j-1))^3)/(6*h(j-1))  h(j)^2/6 h(j+1)^2/6;
    ((-h(j)-h(j-1)-h(j-2))^4-(-h(j)-h(j-1)-h(j-2)-h(j-3))^4)/(24*h(j-3))   ((-h(j)-h(j-1))^4-(-h(j)-h(j-1)-h(j-2))^4)/(24*h(j-2))   ((-h(j))^4-(-h(j)-h(j-1))^4)/(24*h(j-1))  -h(j)^3/24 h(j+1)^3/24;
    ((-h(j)-h(j-1)-h(j-2))^5-(-h(j)-h(j-1)-h(j-2)-h(j-3))^5)/(120*h(j-3))   ((-h(j)-h(j-1))^5-(-h(j)-h(j-1)-h(j-2))^5)/(120*h(j-2))   ((-h(j))^5-(-h(j)-h(j-1))^5)/(120*h(j-1))  h(j)^4/120 h(j+1)^4/120];
w = L\b;
% v = v(end:-1:1);
% v = v/h(j);
q(1) = -v(1);
...
j=N+1;
% b = v(1)*u(j-4)+v(2)*u(j-3)+v(3)*u(j-2)+v(4)*u(j-1)+v(5)*u(j)
% a = w(1)*u(j-3)+w(2)*u(j-2)+w(3)*u(j-1)+w(4)*u(j)+w(5)*u(j+1)
%   X = x(j)+h(j)/2;
%          Y = x(j-1)+h(j-1)/2;
%          
%          c = (a-b)/h(j)
...
d(N+1) = (q(1)*uea(N+2)+q(2)*uea(N+1)+q(3)*uea(N)+q(4)*uea(N-1)+q(5)*uea(N-2)+q(6)*uea(N-3));
