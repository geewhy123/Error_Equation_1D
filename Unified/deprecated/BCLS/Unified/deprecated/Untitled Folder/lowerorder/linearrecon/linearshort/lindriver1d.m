...
for i = 1:N+2
   ...
   % f(i) =(1/h(i))*(2*pi)*(cos(2*pi*(x(i)+h(i)/2))-cos(2*pi*(x(i)-h(i)/2)));
    ...
    %u(i) = (1/h(i))*(1/(2*pi))*(-cos(2*pi*(x(i)+h(i)/2))+cos(2*pi*(x(i)-h(i)/2)));
...
[err(j),Z]=unstructuredrecon1short(u,x,h,N,1,exp(-1));
