...
%   u(i) = (1/h(i))*(sin(pi*(x(i)+h(i)/2))/pi - sin(pi*(x(i)-h(i)/2))/pi);
  ...
  u(i) = (1/h(i))*((x(i)+h(i)/2)^2-((x(i)-h(i)/2)^2))/2;%exp(-(x(i)-0.5)^2);
...
for i = 3:N
 ...
 y= recon1short(x(i),h(i),u(i),x(i-1),h(i-1),u(i-1),x(i+1),h(i+1),u(i+1),i);
   Z(:,i) = y;
   
   
   xx = linspace(x(i)-h(i)/2,x(i)+h(i)/2,M);
   ue = xx;%exp(-xx.^2);%cos(pi*xx);%exp(-(xx-0.5).^2);%xx.^3;%;exp(-(xx-0.5).^2);
   %ue = xx.^3;
   %ue = xx.^4;
   yy = y(1)+y(2)*(xx-x(i));
   plot(xx,yy)
...
y= reconboundary1short(x(i),h(i),u(i),x(i+1),h(i+1),u(i+1),x(i+2),h(i+2),u(i+2),0,'left');
...
xx = linspace(x(i)-h(i)/2,x(i)+h(i)/2,M);
   ...
   plot(xx,yy)
  % ue = xx.^4;
   ue = xx;%exp(-xx.^2);%cos(pi*xx);%exp(-(xx-0.5).^2);%xx.^3;
   error = max(error,max(abs(ue-yy)));
   %upr = y(2)+2*y(3)*h(i)/2+3*y(4)*(h(i)/2)^2;
   %upl = y(2)+2*y(3)*-h(i)/2+3*y(4)*(-h(i)/2)^2;
   %uu(i) = u(i) + k*((upr-upl)/h(i)-f(i)); 
   ...
   i = N+1;
   %%%y= recon1(x(i),h(i),u(i),x(i-1),h(i-1),u(i-1),x(i-2),h(i-2),u(i-2),x(i-3),h(i-3),u(i-3),x(i-4),h(i-4),u(i-4),i);
   y= reconboundary1short(x(i),h(i),u(i),x(i-1),h(i-1),u(i-1),x(i-2),h(i-2),u(i-2),1,'right');
...
%y= recon1short(x(i),h(i),u(i),x(i-1),h(i-1),u(i-1),x(i+1),h(i+1),u(i+1),x(i+2),h(i+2),u(i+2),x(i+3),h(i+3),u(i+3),i);
